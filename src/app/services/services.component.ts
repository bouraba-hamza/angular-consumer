...
export class ServicesComponent{
            ...
            this.produitService.addProduct( this.produitPost ).subscribe(produitPostconsole=>{
                    try {
                        if(typeof produitPostconsole.RefProduit!='undefined'){
                            ...
                            this.addToast({title:'', msg:'Service ajouté avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                        ...
                        console.log(e);
                        this.addToast({title:'', msg:'Service n\'est pas ajouté', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
        ...
        if (this.myformUpdate.valid) {
            ...
            this.produitService.updateProduit( this.produitPost ).subscribe(produitPostconsole=>{
                    try {
                        ...
                        if(typeof produitPostconsole.RefProduit=='undefined'){
                            this.addToast({title:'', msg:'Service modifié avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {

                        this.addToast({title:'', msg:'Service n\'est pas modifié', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
    ...
    delete(produit:Produits):void{
        ...
        this.addToast({title:'', msg:'Service en cours '+ produit.NomProduit, timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
        this.produitService.deleteProduit(produit).subscribe(produitPostconsole=>{

            try {

                if(typeof produitPostconsole.RefProduit =='undefined'){
                    this.addToast({title:'', msg:'Service supprimé avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                    this.myform.reset();
                    setTimeout(() => {this.ngOnInit();}, 1000);
                ...
                console.log(e);
                this.addToast({title:'', msg:'Service n\'est pas supprimé', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        setTimeout(() => {this.ngOnInit();}, 1000);
    }

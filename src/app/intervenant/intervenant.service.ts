...
export class IntervenantService {
  ...
  constructor(private http:HttpClient) { }
    ...
    getPdf(id: number): Observable<any> {
        ...
        return this.http.get(this.ApiUrlPdf+'/'+id).pipe(
    ...
    filter (intervention: Intervenant): Observable<Intervenant> {

      return this.http.post<Intervenant>(this.ApiUrl+'/filter', intervention).pipe(
            tap((intervention: Intervenant) => this.log(`filter w/ id=${intervention.id_intervention}`)),
            catchError(this.handleError<Intervenant>('filter'))
        );
    }

...
export class IntervenantComponent implements OnInit {
    ...
    pageChanged(pN: number): void {
        this.pageNumber = pN;
        // this.isPage = 0;
        // this.intervenantService.pageChange(pN).subscribe(
        //     intervention => {
        //         this.InterventionsData=intervention[0].data;
    ...
    getInterventions():void{
        this.intervenantService.getInterventions().subscribe(intervention => {
            this.InterventionsData=intervention;
        });
    }
    ...
    OnSubmit(){
            ...
            this.intervenantService.addIntervention( this.IntervetionPost ).subscribe(vehiclePostconsole=>{
                    try {
                        ...
                        if (vehiclePostconsole.status == '203') {
                             this.addToast({title:'', msg:"le nombre d'installation supérieur aux boitiérs", timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                         }
                        else {
                            this.addToast({title:'', msg:'Intervention ajoutée avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        ...
                        console.log(e);
                        this.addToast({title:'', msg:'Intervention n\'a pas pu être ajouter', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                }
            );
        }
        else
        {
            ...
            this.addToast({title:'', msg:'veuillez remplir tous les champs du formulaire', timeout:2000, theme:'bootstrap', position:'top-right', type:'error'});
        }
    }
    ...
    onUpdateEtat(id,remarque){
        ...
        if (this.myformUpdate.valid) {
            ...
            this.intervenantService.updateIntervention( this.interventionPost ).subscribe(interventionPostconsole=>{
                    try {
                        this.addToast({title:'', msg:'Etat modifié avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                        setTimeout(() => {this.ngOnInit();}, 1000);

                    }catch (e) {

                        this.addToast({title:'', msg:'Erreur dans la modification de l état d intervention', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                }
            );
        }
    ...
    fileUpload(id,date,customer,startTime,endTime) {
        ...
        this.addToast({title:'', msg:'Fichier modifié avec succès', timeout:1000, theme:'bootstrap', position:'top-right', type:'wait'});

        this.http.post(ApiUrl, formData).subscribe(interventionPostconsole=>{
                console.log(interventionPostconsole);
                this.addToast({title:'', msg:'Fichier modifié avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                setTimeout(() => {this.ngOnInit();}, 1000);
                },
            (error)=>{this.addToast({title:'', msg:'Erreur dans le téléchargement du fichier', timeout:2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        );
        ...
        if (this.myformUpdate.valid) {
            ...
            this.intervenantService.updateIntervention( this.interventionPost ).subscribe(interventionPostconsole=>{
                    try {
                        ...
                        this.addToast({title:'', msg:'Validation du l\'intervention modifiée avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                        setTimeout(() => {this.ngOnInit();}, 1000);

                    }catch (e) {

                        this.addToast({title:'', msg:'Erreur dans la modification de la validation du l intervention', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                }
            );
        }
    ...
    delete(intervention:Intervenant):void {
         ...
         this.addToast({title:'', msg:'En cours'+ intervention.intervention_id, timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
         this.intervenantService.deleteIntervention(intervention).subscribe(vehiculePostconsole=>{
            try {
                ...
                {
                    this.addToast({title:'', msg:'l\'interventervention est déja remplis', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                }
                else{
                ...
                console.log(e);
                this.addToast({title:'', msg:'Erreur dans la suppresion du l\'intervention', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        setTimeout(() => {this.ngOnInit();}, 1000);
    }
    ...
    filter(target,id) {
        this.isGet = 0;
        this.interventionPost=  new Intervenant();
        this.interventionPost.target = target;
        this.interventionPost.data = $('#'+id).val()+'';
        this.intervenantService.filter(this.interventionPost).subscribe(
            intervention => {
                this.InterventionsData=intervention;
    ...
    alertClient(){
        this.addToast({title:'', msg:'Vous devez choisir le client !!!', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
    }


    file_change(){
        $('#uplaod_div_file p').html('Image ajoutée avec succès !!');
    }
}

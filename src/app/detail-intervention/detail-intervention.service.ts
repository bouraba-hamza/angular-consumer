...
export class DetailInterventionService {
  ...
  constructor(private http:HttpClient) { }
    ...
    getSimOnInit(intervention: Intervenant): Observable<Intervenant> {

      return this.http.post<Intervenant>(this.ApiUrl+'/getSim', intervention).pipe(
            tap((intervention: Intervenant) => this.log(`GetSim w/ id=${intervention.id_intervention}`)),
            catchError(this.handleError<Intervenant>('GetSim'))
        );
    }

    getBoxOnInit(intervention: Intervenant): Observable<Intervenant> {

      return this.http.post<Intervenant>(this.ApiUrl+'/getBox', intervention).pipe(
            tap((intervention: Intervenant) => this.log(`GetSim w/ id=${intervention.id_intervention}`)),
            catchError(this.handleError<Intervenant>('GetSim'))
        );
    }

    updateStatus(intervention: Intervenant): Observable<Intervenant> {

      return this.http.post<Intervenant>(this.ApiUrl+'/store_in_session', intervention).pipe(
            tap((intervention: Intervenant) => this.log(`store_in_session w/ id=${intervention.id_intervention}`)),
            catchError(this.handleError<Intervenant>('store_in_session'))
        );
    }
    ...
    getRefMovement(categorie){
      ...
      return this.http.get<Intervenant[]>(this.ApiUrl+'/movement/'+categorie );
    }

    getPlan(categorie){

      return this.http.get<Intervenant[]>(this.ApiUrl+'/plan/'+categorie );
    }

...
export class ProduitsComponent {
             ...
             this.produitService.addProduct( this.produitPost ).subscribe(produitPostconsole=>{
                try {
                    if(typeof produitPostconsole.RefProduit!='undefined'){
                        this.addToast({title:'', msg:'Votre Produit est ajouté à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                        this.myform.reset();
                        setTimeout(() => {this.ngOnInit();}, 1000);
                    }
                }catch (e) {
                    this.addToast({title:'', msg:'Votre Produit n\'est pas ajouté à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                } }
        );}
    }
       ...
       if (this.myformUpdate.valid) {
           ...
           this.produitService.updateProduit( this.produitPost ).subscribe(produitPostconsole=>{
                   try {
                       ...
                       if(typeof produitPostconsole.RefProduit=='undefined'){
                           this.addToast({title:'', msg:'Votre Produit est modifié à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                           this.myform.reset();
                           setTimeout(() => {this.ngOnInit();}, 1000);
                       }
                   }catch (e) {

                       this.addToast({title:'', msg:'Votre Produit n\'est pas modifié à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                   }}
           );}
    }
    ...
    delete(produit:Produits):void{
        ...
        this.produitService.deleteProduit(produit).subscribe(produitPostconsole=>{
            try {
                this.addToast({title:'', msg:'Votre Produit est en cours', timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
                if(typeof produitPostconsole.RefProduit =='undefined'){
                    this.addToast({title: '', msg:'Votre Produit est supprimé', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                    this.myform.reset();
                    setTimeout(() => {this.ngOnInit();}, 1000);
                }
            }catch (e) {
                ...
                this.produitService.UpdateEtat(produit).subscribe(produitPostconsole=>{
                    try {
                        if(typeof produitPostconsole.RefProduit =='undefined'){
                            this.addToast({title:' ', msg:'Votre Produit est Hore Vente Hors Achat', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {
                        ...
                        this.addToast({title:'Votre produit ne peut pas être supprimer à votre sustème', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                });

  ...
  constructor(private banqueService: BanqueService, private http: HttpClient, private modalService: NgbModal, private toastyService: ToastyService) { }
    ...
    addToast(options) {
     ...
     onUpdate(id,RefCompte,Type,Devis,Domiciliation,Nom,Numero,Proprietaire,Solde){
         ...
         if (this.myformUpdate.valid) {
             this.addToast({title:'', msg:'En cours ', timeout:1000, theme:'bootstrap', position:'top-right', type:'wait'});
             ...
             this.banqueService.updateCompte( this.banquePost ).subscribe(comptePostconsole=>{
                 ...
                 try {
                     ...
                     if(typeof comptePostconsole.RefCompte=='undefined'){
                         this.addToast({title:'', msg:'Votre Compte est modifié dans votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
            ...
            this.banqueService.addCompte( this.banquePost ).subscribe(comptePostconsole=>{
                    try {
                        console.log(comptePostconsole);
                            this.addToast({title:'', msg:'Votre compte est ajouté avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myformCompte.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                    }catch (e) {

                        this.addToast({title:'', msg:'Erreur dans l\'ajout du compte', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                }
            );
        }
    ...
    delete(compte:Banque):void{
        ...
        this.banqueService.deleteCompte(compte).subscribe(comptePostconsole=>{
            try {
                this.addToast({title:'', msg:'Votre Compte est en cours', timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
                if(typeof comptePostconsole.RefCompte =='undefined'){
                    this.addToast({title:'', msg:'Votre Compte est supprimé', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});

                    setTimeout(() => {this.ngOnInit();}, 1000);
                }
            }catch (e) {

                this.addToast({title:'', msg:'Votre compte n\'a pas pu être supprimer', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});

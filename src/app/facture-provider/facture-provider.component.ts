            ...
            this.factureProviderService.addFacture( this.facturePost ).subscribe(facturePostconsole=>{
                    try {
                        if(typeof facturePostconsole.RefFacture!='undefined'){
                            this.addToast({title:'', msg:'Votre Facture est ajouté à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {
                        this.addToast({title:'', msg:'Votre Facture n\'est pas ajouté à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
            ...
            this.factureProviderService.addLignesFacture( this.ligneFacturePost ).subscribe(factureLignePostconsole=>{
                    try {
                        if(typeof factureLignePostconsole.TVA!='undefined'){
                            ...
                            this.addToast({title:'', msg:'Votre Ligne Facture est en cours', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myformligne.reset();
                            this.getLigneFacture(id);
                            setTimeout(() => {this.ngOnInit();}, 10);
                        }
                    }catch (e) {
                        this.addToast({title:'Votre Ligne Facture n\'est pas ajouté à votre système', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }

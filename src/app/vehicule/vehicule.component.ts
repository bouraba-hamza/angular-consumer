...
export class VehiculeComponent implements OnInit {
  ...
  constructor(private vehiculeService: VehiculeService,private http: HttpClient,private modalService: NgbModal,private toastyService: ToastyService) { }
        ...
        if (this.myformUpdate.valid) {
            ...
            this.vehiculeService.updateVehicle( this.vehiculePost ).subscribe(vehiclePostconsole=>{
                    try {
                            ...
                            this.addToast({title:'', msg:'Véhicule modifié avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            //this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);

                    }catch (e) {

                        this.addToast({title:'', msg:'Véhicule n\'est pas modifié', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                }
            );
        }
        ...
        if (this.myform.valid) {
            ...
            this.vehiculeService.addVehicule( this.vehiculePost ).subscribe(vehiclePostconsole=>{
                    try {
                            ...
                            this.addToast({title:'', msg:'Véhicule ajouté avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        ...
                        console.log(e);
                        this.addToast({title:'', msg:'Véhicule n\'est aps ajouté', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                }
            );
        }
    ...
    delete(vehicule:Vehicule):void{
        ...
        this.addToast({title:'', msg:'véhicule en cours '+ vehicule.id, timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
        this.vehiculeService.deleteVehicule(vehicule).subscribe(vehiculePostconsole=>{

            try {

              //  if(typeof vehiculePostconsole.name=='undefined'){
                    this.addToast({title:'', msg:'Véhicule supprimé avec succès', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                    //this.myform.reset();
                    setTimeout(() => {this.ngOnInit();}, 1000);
                ...
                console.log(e);
                this.addToast({title:'', msg:'Véhicule n\'est aps supprimé', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        setTimeout(() => {this.ngOnInit();}, 1000);
    }

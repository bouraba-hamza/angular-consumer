            ...
            this.reglementProviderService.addFacture( this.facturePost ).subscribe(facturePostconsole=>{
                    try {
                        if(typeof facturePostconsole.RefFacture!='undefined'){
                            this.addToast({title:'', msg:'Votre Facture est ajoutée à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {
                        this.addToast({title:'', msg:'Votre Facture n\'est pas ajoutée à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
        ...
        if (this.myformUpdate.valid) {
            ...
            this.reglementProviderService.updateFacture( this.facturePost ).subscribe(facturePostconsole=>{
                    try {
                        ...
                        if(typeof facturePostconsole.RefFacture=='undefined'){
                            this.addToast({title:'', msg:'Votre Facture est modifiée à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {
                        this.addToast({title:'', msg:'Votre Facture n\'est pas modifiée à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                    });
        }
    }
    ...
    delete(facture:Factures):void{
        ...
        this.reglementProviderService.deleteFacture(facture).subscribe(produitPostconsole=>{
            ...
            try {
                ...
                console.log(e);
                this.addToast({title:'', msg:'Votre Facture n\'est pas supprimée à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        setTimeout(() => {this.ngOnInit();}, 1000);
    }

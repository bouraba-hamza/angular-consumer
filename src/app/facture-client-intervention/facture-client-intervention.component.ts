   ...
   test2 : Factures;
    ...
    pdfFactureClientIntervention(id:number):void{
        this.factureClientService.FacturePdf(id).subscribe(facture => {
            console.log('test');
            console.log(facture);
            window.open('http://192.168.3.111:8000/api/FactureClientIntervention/pdf/'+id,"_self")
    ...
    delete(facture:Factures):void{
        ...
        this.factureClientService.deleteFacture(facture).subscribe(produitPostconsole=>{
            ...
            try {
                ...
                console.log(e);
                this.addToast({title:'', msg:'Votre Facture n\'a pas pu être supprimer', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        setTimeout(() => {this.ngOnInit();}, 1000);
    }
    ...
    deleteLigne(ligneFacture:LigneFacture):void{
        ...
        this.factureClientService.deleteLignesFacture(ligneFacture).subscribe(produitPostconsole=>{
            ...
            try {
                ...
                console.log(e);
                this.addToast({title:'', msg:'Votre Ligne Facture n\'a pas pu être supprimer', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        ...
        if (this.myformligne.valid) {
            ...
            this.factureClientService.UpdateLignesFacture( this.ligneFacturePost ).subscribe(factureLignePostconsole=>{
                    try {
                            ...
                            this.addToast({title:'', msg:'Votre Facture est ajoutée', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                        ...
                        this.addToast({title:'', msg:'Votre  Facture n\'a pas pu être ajouter', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }

...
export class DevisClientComponent {
   ...
   test2 : Devis;
        ...
        if (this.myform.valid) {
           ...
           this.devisClientService.addDevis(this.devisPost).subscribe(devisPostconsole => {
                    try {
                        ...
                        this.addToast({
                            title: '',
                            msg: 'Votre Devis est ajouté à votre système',
                            timeout: 2000,
                            theme: 'bootstrap',
                            position: 'top-right',
                            type: 'success'
                        ...
                        this.addToast({
                            title: '',
                            msg: 'Votre Devis n\'est pas ajouté à votre système',
                            timeout: 2000,
                            theme: 'bootstrap',
                            position: 'top-right',
                            type: 'error'
            ...
            this.devisClientService.updateDevis( this.devisPost ).subscribe(devisPostconsole=>{
                    try {
                        ...
                        if(typeof devisPostconsole.id=='undefined'){
                            this.addToast({title:'Votre devis est modifié à votre système', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {
                        this.addToast({title:'Votre devis n\'est pas modifié à votre système', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
                    });
        }
    }
    ...
    delete(devis:Devis):void{
        ...
        this.addToast({title:'', msg:'Votre Devis est en cours', timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
        this.devisClientService.deleteDevis(devis).subscribe(produitPostconsole=>{

            try {

                if(typeof produitPostconsole.RefFacture =='undefined'){
                    this.addToast({title:'', msg:'Votre Devis est supprimé', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                    setTimeout(() => {this.ngOnInit();}, 1000);
                }
            }catch (e) {
                console.log(e);
                this.addToast({title:'', msg:'Votre devis n\'a pas pu être supprimer à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
            }
        });
        setTimeout(() => {this.ngOnInit();}, 1000);
    }
    ...
    deleteLigne(ligneDevis:LigneDevis):void{
        ...
        this.addToast({title:'', msg:'Votre Ligne Devis est en cours', timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
        this.devisClientService.deleteLignesDevis(ligneDevis).subscribe(produitPostconsole=>{
            ...
            try {
                if(typeof produitPostconsole.id =='undefined'){
                    ...
                    this.addToast({title:' ', msg:'Votre Ligne Devis est supprimé', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
        ...
        if (this.myformligne.valid) {
            ...
            this.devisClientService.addLignesDevis( this.ligneDevisPost ).subscribe(devisLignePostconsole=>{
                    try {
                            ...
                            this.addToast({title:'Votre ligneDevis est en cours', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                            this.myformligne.reset();
                            this.getLigneDevis(id);
                            setTimeout(() => {this.ngOnInit();}, 10);

                    }catch (e) {
                        this.addToast({title:'', msg:'Votre ligneDevis n\'a pas pu être supprimer à votre système', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                    }
